...
def parseargs():
    ...
    parser.add_argument("-f", "--fasta_output", help="If used output will be a SNP alignment fasta file, if not output will be a position by position matrix", action='store_true')
...
def get_loci_info(args,conn):
    pqsl_query = """SELECT "identifier","start_location","end_location","orientation" from "{}_locus";""".format(args.appname)
...
def main():
    ...
    ap_snp_align,refseq,outrefpos = make_snpalign(ap2all,loci_pos_dict,snpinfo,mgt9loci,locusfiles,loc_to_snppos,loc_snp_ref)
