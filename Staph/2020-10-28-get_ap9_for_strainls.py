...
##python /Users/liamcheneyy/PycharmProjects/Python_PhD/Staph/2020-10-28-get_ap9_for_strainls.py --appname Staphylococcus --dbname saureus_4 -o /Users/liamcheneyy/Desktop/USA300-NAR-alleles.txt -u postgres -p 5678 -f /Users/liamcheneyy/Desktop/strains.txt
...
def getSts(args,conn,strainlist=[]):
    """
...
def parseargs():
    ...
    parser.add_argument("--dbname", help="database name ", default="salmonella")
    ...
    parser.add_argument("-u", "--user", help="postgres user", default='mlstwebsite')
    ...
    group.add_argument('--all',
                       help="get all data in db. Mutually exclusive to -l,-f,--seqtype.--cc", action='store_true')
...
def main():
    ...
    DbConString = "dbname={} host='{}' port='{}' user='{}' password='{}'".format(args.dbname,args.host,args.port,args.user,args.password)  ## connection info for Db - assign new user that can only do what is needed for script
    ...
    if not args.all:
        ...
        reslists = getSts(args, conn, strainlist)
    else:
        reslists = getSts(args, conn)
