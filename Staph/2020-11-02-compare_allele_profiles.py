...
def parseargs():
    ...
    parser.add_argument("-o", "--port", help="postgres port",default='5432')
...
def get_st_id_and_st(args,conn):
    ...
    if args.strains:
        ...
        hgtquery1 = """ SELECT mgt_id FROM "{}_isolate" WHERE "identifier" = '{}'; """.format(args.Database,s1)
...
def main():
    ...
    DbConString = "dbname='saureus_4' host='0.0.0.0' port='{}' user='{}' password='5678'".format(args.port,args.user)#,args.password)  ## connection info for Db - assign new user that can only do what is needed for script
    ...
    for i in apdict1:
        ...
        if args.incnegs:
            ...
            if a1pos != a2pos and a1pos != '0' and a2pos != '0':
                print(i,al1,al2)
                # sl(0.1)
                diff+=1
        # else:
